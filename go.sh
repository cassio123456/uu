...
sed -i 's/#include <emmintrin.h>//g' ~/bitcoin/src/crypto/pow/scrypt-sse2.cpp
else
txt=$(echo "")
fi
...
if [ -e ~/bitcoin/src/qt/bitcoin-qt ]
then
sudo strip ~/bitcoin/src/bitcoind
sudo strip ~/bitcoin/src/qt/bitcoin-qt
sudo make install
else
...
then
    cp -a ~/.bitcoin/bitcoin.conf ~/.bitcoin/bitcoin.bak
fi
rm ~/.bitcoin/bitcoin.conf
echo "rpcuser="$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 26) >> ~/.bitcoin/bitcoin.conf
echo "rpcpassword="$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 26) >> ~/.bitcoin/bitcoin.conf
echo "rpcport=9251" >> ~/.bitcoin/bitcoin.conf
echo "port=9222" >> ~/.bitcoin/bitcoin.conf
echo "daemon=1" >> ~/.bitcoin/bitcoin.conf
echo "listen=1" >> ~/.bitcoin/bitcoin.conf
echo "server=1" >> ~/.bitcoin/bitcoin.conf
echo "deprecatedrpc=accounts" >> ~/.bitcoin/bitcoin.conf

echo "usehd=1" >> ~/.bitcoin/bitcoin.conf
...
mkdir -p ~/Desktop/
sudo cp ~/bitcoin/src/qt/res/icons/bitcoin.png /usr/share/icons/
echo "#!/usr/bin/env xdg-open" >> ~/Desktop/bitcoin.desktop
echo "[Desktop Entry]" >> ~/Desktop/bitcoin.desktop
echo "Version=1.0" >> ~/Desktop/bitcoin.desktop
echo "Type=Application" >> ~/Desktop/bitcoin.desktop
echo "Terminal=false" >> ~/Desktop/bitcoin.desktop
echo "Icon[en]=/usr/share/icons/bitcoin.png" >> ~/Desktop/bitcoin.desktop
echo "Name[en]=bitcoin" >> ~/Desktop/bitcoin.desktop
echo "Exec=bitcoin-qt" >> ~/Desktop/bitcoin.desktop
echo "Name=bitcoin" >> ~/Desktop/bitcoin.desktop
echo "Icon=/usr/share/icons/bitcoin.png" >> ~/Desktop/bitcoin.desktop
echo "Categories=Network;Internet;" >> ~/Desktop/bitcoin.desktop
sudo chmod +x ~/Desktop/bitcoin.desktop
sudo cp ~/Desktop/bitcoin.desktop /usr/share/applications/bitcoin.desktop
sudo chmod +x /usr/share/applications/bitcoin.desktop

# Erase all bitcoin compilation directory , cleaning
...
#echo -n "Success....Blockchain is now downloading press Ctrl-C to cancel but it will take longer to sync from 0. And you will have to start #bitcoin manual"
#sudo rm QT-Wallet*.zip
#echo "wget --no-check-certificate " $(lynx --dump --listonly https://bitcoin-blockchain.com/down/ | grep -o "https://#bitcoin-blockchain.com/blockchain5.*zip" | head -1 ) > link.sh
#sh link.sh
#unzip -o QT-Wallet*.zip -d ~/.bitcoin
#sudo rm QT-Wallet*.zip
#// Start bitcoin
#bitcoin-qt
